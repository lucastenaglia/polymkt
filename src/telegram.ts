...
import { Position, TradeNotificationData, ClosedTradeNotificationData } from './types';
...
export async function sendClosedTradeNotification(data: ClosedTradeNotificationData) {
    if (!config.telegramChatId) return;

    const emoji = data.status === 'WON' ? '✅' : (data.status === 'LOST' ? '❌' : '📉');
    const statusText = data.status === 'WON' ? 'PROFIT' : (data.status === 'LOST' ? 'LOSS' : 'CLOSED');
...
📊 *Market*: ${data.marketSlug || data.marketId}
⚖️ *Outcome*: ${data.outcome}
💰 *Amount*: $${data.amountUsd.toFixed(2)}
📈 *Entry*: $${data.entryPrice.toFixed(4)}
📉 *Exit*: $${data.exitPrice.toFixed(4)}

💵 *PnL*: ${pnlSign}$${data.pnl.toFixed(2)} (${pnlSign}${data.pnlPercentage.toFixed(2)}%)
👤 *Target*: ${data.targetName || data.targetUser}
`;
    ...
    try {
        ...
        if (error.code === 'ETIMEDOUT' || error.message?.includes('timeout')) {
            console.warn('[TELEGRAM] Close notification timed out.');
        } else {
            console.warn('[TELEGRAM] Failed to send close notification:', error.message);
        }
    }
}
